...
export async function GET(
  ...
  try {
    ...
    const { data: botsRows } = await supabase
      .from('meme_bots')
      .select('id, slot_order, action, fee_pct, bot_wallet, label, destination_wallet, last_run_at, total_sol_spent, total_tokens_acted, expires_at, created_at')
      .eq('meme_id', id)
    ...
    return NextResponse.json({
      meme: {
        ...
        // Migration 053 — quote currency. Defaults to 'sol' for legacy

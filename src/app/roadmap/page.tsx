...
const SHIPPED = [
  { icon: Coins, title: 'Per-meme buyback bot (fee-delegation model)', desc: 'Creator picks a % of trading fees to route to a system-controlled bot wallet at submission time. The bot auto-buys their token on Jupiter and burns or holds the bought supply. Fully on-chain, every swap and burn auditable. Creator dials supply pressure without giving up backer yield they didn\'t intend to.' },
  ...
  { icon: Shield, title: 'Reserved slots + TEAM ROUND', desc: 'Creators can reserve N of M slots for specific wallets without hiding the launch from public. Hybrid mode shows "X open · Y reserved" so backers see exactly what\'s available; fully-reserved (TEAM ROUND) launches get a distinct amber label so visitors know upfront. Public always sees the launch — that\'s the brand promise.' },
  ...
  { icon: Shield, title: 'Auto-refund on rejected backings', desc: 'If the server rejects a backing after the on-chain deposit landed (cap exceeded, slot taken in a race, reservation gate), the deposit is auto-refunded from the pool wallet back to the backer in the same API response — no support tickets, no stranded SOL. The whole rejection-recovery path is one atomic flow.' },
...
const BUILDING = [
  { icon: Shield, title: 'Structured Team Round v2 (disclosure + auto-vesting)', desc: 'Phase 7 shipped reserved slots + TEAM ROUND label. Phase 8 turns that into a real product: require team identity declaration at submission, auto-create a Streamflow vesting contract for team-backer allocations, surface the disclosure publicly on the token page during the gated round. Teams get the structure they need; backers get certainty team tokens are locked for N days. Pump.fun doesn\'t offer this.' },
  { icon: Coins, title: 'Buyback bot: distribute actions', desc: 'Buyback bot v1 supports burn + hold. Adds distribute_holders (snapshot current holders, transfer bought tokens pro-rata) and distribute_backers (pro-rata transfer to genesis backers). Same fee-delegation primitive, two new payout shapes for creators who want recurring holder rewards instead of just deflation.' },

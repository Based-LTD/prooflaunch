...
export function ConnectButton() {
  ...
  // The balance rides in the chip on purpose. With several wallet
  // extensions in one browser the site can end up connected to an account
  // the backer never meant to use, and an address alone hides that — one
  // backer only found out when the signature failed (2026-09-25). A
  // visible 0.0000 gives it away before anything is clicked.
  const { data: bal } = useBalance({ address, chainId: robinhoodChain.id, query: { enabled: !!address } });
  const empty = !!bal && bal.value === 0n;
  ...
  return (
    <button
      onClick={() => disconnect()}
      className={`px-2.5 sm:px-4 py-2.5 text-xs font-mono border transition-colors ${
        empty
          ? 'border-[var(--warning)]/60 text-[var(--warning)] hover:border-[var(--error)] hover:text-[var(--error)]'
          : 'border-[var(--border)] text-[var(--foreground)] hover:border-[var(--error)] hover:text-[var(--error)]'
      }`}
      title={empty
